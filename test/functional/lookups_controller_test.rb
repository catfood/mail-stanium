...
class LookupsControllerTest < ActionController::TestCase
  ...
  test "should create lookup" do
    assert_difference('Lookup.count') do
      post :create, lookup: { destination: @lookup.destination, match: @lookup.match, sorttype: @lookup.sorttype }
    end
  ...
  test "should update lookup" do
    put :update, id: @lookup, lookup: { destination: @lookup.destination, match: @lookup.match, sorttype: @lookup.sorttype }
    assert_redirected_to lookup_path(assigns(:lookup))
  end
